...
int main( int argc, char * argv[] )
{
    ...
    int obj1_x = 100;
    int obj1_y = 200;
    //int obj1_init_y = obj1_y;
    int obj1_init_x = obj1_x;
    int obj1_init_y = obj1_y;
    int obj1_final_x = 1800;
    int obj1_final_y = 200;
    ...
    /*while(time <= total){
        obj1_x = ease_bounceOut(100, 1500, time, total);
        cout<<time<<endl;
        //obj1_x += 100;
        filledCircleRGBA(m_renderer, obj1_x, obj1_y, 50, 210, 50, 35, 255);
        SDL_RenderPresent(m_renderer);
        time++;
        SDL_Delay(1000 / room_speed);
        SDL_SetRenderDrawColor( m_renderer, 0, 100, 0, 255 );
        SDL_RenderClear( m_renderer );
    }*/
    /*e->type = 0;
    ...
    int y0 = 600;
    for(int i = 1; i <= n; i++){
        aalineRGBA(m_renderer,x0, y0 - 50 * (n - i), x0 + 50 * i, y0,255,255,255,255);
    }*/

    SDL_RenderPresent(m_renderer);
    int hsp = 10, vsp = 10, x = 50, y = 50, r = 50;
    e->type = 0;
    while(e->type != SDL_KEYDOWN){
            SDL_PollEvent(e);
            filledEllipseRGBA(m_renderer, x, y, r, r, 255, 255, 255, 255);
            if(x > W - r || x < r)  hsp = hsp;
            if(y > H - r || y < r)  vsp = -vsp;
            SDL_RenderPresent(m_renderer);
            SDL_Delay(1000 / room_speed);
            SDL_RenderClear( m_renderer );
            SDL_SetRenderDrawColor( m_renderer, 0, 100, 0, 255 );
